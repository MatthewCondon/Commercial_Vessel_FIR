...
local heading_deg = ProtoField.float("nmea-2000-127250.heading_deg", "Heading (deg)")
...
NMEA_2000_127250.fields = {sid,heading,heading_deg,deviation,variation,reference}
function NMEA_2000_127250.dissector(buffer, pinfo, tree)
    ...
    if buffer:len() >= (str_offset + 1 + 2) then
        local heading_rad = buffer(str_offset + 1, 2):le_uint() * 0.0001
        subtree:add(heading, buffer(str_offset + 1, 2), heading_rad)
        subtree:add(heading_deg, buffer(str_offset + 1, 2), heading_rad * (180 / math.pi))
    end
