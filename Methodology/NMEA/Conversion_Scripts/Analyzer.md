...
## Collection
Converting to the Analyzer format requires live data collection from the NMEA backbone. A connection to the can interface must be established before data can be sent:
```
sudo ip link set can0 up type can bitrate 250000 restart-ms 100
```
The command to then collect and save the data to an output file is:
```
